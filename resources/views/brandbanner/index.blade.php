        ...
        <div class="mx-auto p-6">
            ...
            <div class="bg-white shadow-sm rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div class="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50/50">
                    ...
                    <a href="{{ route('brandbanners.create') }}"
                       class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl transition-all duration-200 shadow-xs text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20 active:scale-[0.98]">
                        ...
                        <span>Add Brands Banner</span>
                    </a>
                </div>

                <!-- Filter / Search Form -->
                <form method="GET" action="{{ route('brandbanners.index') }}" class="p-6">
                ...
                <div class="px-6 pb-6 overflow-x-auto">
                    ...
                    <!-- Table with Outer Border, Sharp Edges, and table-fixed for balanced columns -->
                    <table class="w-full text-left border-collapse border border-gray-200 table-fixed">
                        <!-- Table Head -->
                        <thead>
                        <tr class="bg-gray-50 border-b border-gray-200 text-gray-700 text-xs uppercase font-semibold">
                            ...
                            <th class="px-4 py-3.5 border-r border-gray-200 w-44">Name</th>
                            <th class="px-4 py-3.5 border-r border-gray-200 w-72">Description</th> <!-- Yahan w-72 fix width de di hai -->
                            <th class="px-4 py-3.5 border-r border-gray-200 w-36">Button Name</th>
                            <th class="px-4 py-3.5 border-r border-gray-200 w-32">Created At</th>
                        ...
                        @forelse($brandsBanners as $key => $banner)

                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">

                                <td class="px-4 py-3.5 border-r border-gray-200 text-gray-500 font-medium">
                                    {{ $brandsBanners->firstItem() + $key }}
                                </td>

                                <td class="px-4 py-3.5 border-r border-gray-200">
                                    @if($banner->image)
                                        <img src="{{ asset('storage/' . $banner->image) }}" class="h-12 w-16 object-cover rounded-lg border border-gray-200 shadow-xs">
                                    @else
                                ...
                                <td class="px-4 py-3.5 border-r border-gray-200 font-semibold text-gray-800">
                                    <div class="truncate" title="{{ $banner->name }}">
                                        {{ $banner->name }}
                                    </div>
                                </td>

                                <!-- Description with fixed width and truncate/line-clamp -->
                                <td class="px-4 py-3.5 border-r border-gray-200 text-gray-600">
                                    <div class="truncate" title="{{ $banner->description }}">
                                        {{ $banner->description }}
                                    </div>
                                </td>

                                <!-- Button Name with truncate -->
                                <td class="px-4 py-3.5 border-r border-gray-200">
                                    <div class="truncate" title="{{ $banner->button_name ?? '-' }}">
                                        {{ $banner->button_name ?? '-' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-2">
                                        <!-- View (Show) Button with Eye Icon -->
                                        <a href="{{ route('brandbanners.show', $banner->id) }}"
                                           ...
                                           title="View Banner Settings">
                                            <i class="fa-solid fa-eye text-xs"></i>
                                        </a>
                                        <!-- Edit Button -->
                                        <a href="{{ route('brandbanners.edit', $banner->id) }}"
                                        ...
                                        <form action="{{ route('brandbanners.destroy', $banner->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                                    ...
                                    <span>No Brands Banners Found</span>
                                </td>
                            </tr>
                    ...
                    @if($brandsBanners->hasPages())
                        <div class="pt-4 border-t border-gray-200 mt-4">
                            {{ $brandsBanners->appends(request()->query())->links() }}
                        </div>
                    @endif
