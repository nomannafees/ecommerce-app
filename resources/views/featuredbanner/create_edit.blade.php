            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">
                        {{ !empty($featuredBanner) ? 'Edit Featured Banner' : 'Create Featured Banner' }}
                    </h2>
                </div>
                ...
                <a href="{{ route('featuredbanners.index') }}"
                   class="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl transition duration-300 shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/50">
                    ...
                    <span>List Banners</span>
                </a>
            </div>

            <!-- Form -->
            <form action="{{ !empty($featuredBanner->id) ? route('featuredbanners.update', $featuredBanner->id) : route('featuredbanners.store') }}" method="POST" enctype="multipart/form-data">

                @csrf

                @if(!empty($featuredBanner))
                    @method('PUT')
                @endif
                ...
                <div class="grid gap-6 mb-8 md:grid-cols-2">
                    ...
                    <div class="relative md:col-span-2">
                        ...
                        <input type="text"
                               ...
                               id="banner_name"
                               required
                               value="{{ old('name', $featuredBanner->name ?? '') }}"
                               placeholder=" "
                    ...
                    <!-- Button Name (Full Width or Half) -->
                    <div class="relative md:col-span-2">
                        <input type="text"
                               name="button_name"
                               id="button_name"
                               value="{{ old('button_name', $featuredBanner->button_name ?? '') }}"
                               placeholder=" "
                               class="peer w-full px-4 py-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200">

                        <label for="button_name"
                               class="absolute left-4 top-3.5 text-gray-400 text-sm pointer-events-none transition-all duration-200
                               peer-focus:text-xs peer-focus:text-emerald-600 peer-focus:-translate-y-6 peer-focus:left-3 peer-focus:bg-white peer-focus:px-2
                               peer-[:not(:placeholder-shown)]:-translate-y-6 peer-[:not(:placeholder-shown)]:left-3 peer-[:not(:placeholder-shown)]:bg-white peer-[:not(:placeholder-shown)]:px-2 peer-[:not(:placeholder-shown)]:text-xs">
                    ...
                    <div class="relative md:col-span-2">
                        ...
                        <textarea name="description"
                                  ...
                                  class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200">{{ old('description', $featuredBanner->description ?? '') }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <!-- Image Preview Box -->
                        <div id="image_preview_container" class="{{ !empty($featuredBanner->image) ? '' : 'hidden' }} mt-4 flex items-center gap-3">
                            <span id="preview_label" class="text-xs text-gray-500 font-medium">
                                {{ !empty($featuredBanner->image) ? 'Current Image:' : 'Selected Preview:' }}
                            </span>
                            <div class="inline-flex items-center justify-center h-20 w-32 p-1 bg-white border border-gray-200 rounded-xl shadow-sm">
                                <img id="banner_image_preview"
                                     src="{{ !empty($featuredBanner->image) ? asset('storage/' . $featuredBanner->image) : '#' }}"
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit"
                        ...
                        <span>{{ !empty($featuredBanner) ? 'Update' : 'Save' }}</span>
                    </button>
                </div>
