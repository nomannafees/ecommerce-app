@forelse($products as $product)
    ...
    <div
        class="group relative bg-white rounded-md sm:rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition duration-300 flex flex-col h-full w-full">
        <!-- WISHLIST BUTTON -->
            ...
            <div class="p-1.5 sm:p-2.5 xs:p-2.5 md-p-2.5 lg-p-2.5 xl-p-2.5 2xl-p-2.5 flex-grow flex flex-col justify-between gap-2">
                ...
                <div class="flex items-center justify-between gap-2 -mt-1">
                    ...
                    <div class="flex flex-col">
                        @if($variant)
                            <span
                                class="text-xs sm:text-base font-bold text-green-600 whitespace-nowrap">Rs {{ number_format($variant->price) }}</span>
                        ...
                        @else
                            <span
                                class="text-xs sm:text-base font-bold text-gray-500 whitespace-nowrap">Rs {{ number_format($product->base_price ?? 0) }}</span>
                        @endif
                    </div>
