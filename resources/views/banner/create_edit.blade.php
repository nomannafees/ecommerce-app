...
@section('content')
    ...
    <div class="mx-auto py-6 px-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-8">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">
                        {{ !empty($banner) ? 'Edit Banner' : 'Create Banner' }}
                    </h2>
                </div>
            ...
            <!-- Form -->
            <form action="{{ !empty($banner->id) ? route('banners.update', $banner->id) : route('banners.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-6 mb-8 md:grid-cols-2">
                    ...
                    <!-- Banner Name (Floating Label) - FULL WIDTH -->
                    ...
                    <div class="relative md:col-span-2">
                        ...
                        <textarea name="description"
                                  ...
                                  class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200">{{ old('description', $banner->description ?? '') }}</textarea>
                    ...
                    <!-- Sort Order Dropdown - FULL WIDTH -->
                    <div class="relative md:col-span-2">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 peer-focus:text-emerald-600 transition-colors duration-200 pointer-events-none">
                        ...
                        <select name="sort_order"
                                id="sort_order"
                                class="peer w-full pl-11 pr-4 py-3.5 border border-gray-200 rounded-xl bg-gray-50/50 text-gray-800 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200">
                            <option value="1" {{ old('sort_order', $banner->sort_order ?? '') == 1 ? 'selected' : '' }}>1 - First Banner (Large Left Side)</option>
                            <option value="2" {{ old('sort_order', $banner->sort_order ?? '') == 2 ? 'selected' : '' }}>2 - Second Banner (Top Right)</option>
                            <option value="3" {{ old('sort_order', $banner->sort_order ?? '') == 3 ? 'selected' : '' }}>3 - Third Banner (Bottom Right)</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <!-- Image Preview Box -->
                        <div id="image_preview_container" class="{{ !empty($banner->image) ? '' : 'hidden' }} mt-4 flex items-center gap-3">
                            <span id="preview_label" class="text-xs text-gray-500 font-medium">
                                {{ !empty($banner->image) ? 'Current Image:' : 'Selected Preview:' }}
                            </span>
                            <div class="inline-flex items-center justify-center h-20 w-32 p-1 bg-white border border-gray-200 rounded-xl shadow-sm">
                                <img id="banner_image_preview"
                                     src="{{ !empty($banner->image) ? asset('storage/' . $banner->image) : '#' }}"
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit"
                        ...
                        <span>{{ !empty($banner) ? 'Update' : 'Save' }}</span>
                    </button>
                </div>
    ...
    <!-- Scripts for Live Image Preview -->
    <script>
